...
{    
  ...
  // Самозагрузчик
  uint16_t selfLoader_mem_pages_cnt = (FLASH1986VE9x_PAGES_CNT - LOADERS_COUNT) - 2 - 1;     // кол-во страниц для прошивки по одной странице отводится для загрузчиков и 2 для учета наработки и одна для настроек во флешке
...
__ramfunc void change_loader_mode(Loader_TypeDef *loader, uint8_t newState)
{
   ...
   uint32_t nvic_iser = NVIC->ISER[0];  // разрешенные прерывания
   if(loader->state == CANLOADER_STATE_ACTIVE)                           // вход в режмм программирования
   {    
      //NVIC->ICER[0] = 0xFFFFFFFF;                    // запрет прерываний  
      //NVIC_EnableIRQ(Timer2_IRQn);                   // разрешаем прерывания системного таймера для отсчета времени
      __disable_irq();
   ...
   else 
   {
      ...
      __enable_irq();
      //NVIC->ISER[0] = nvic_iser;            // разрешение прерываний
