...
void can2_init()
{
   ...
   _can2_initRxBuffer(CAN_MONITOR_RX_BUF, CAN_RX_FILTER(CANID_TERM_RX));        // монитор
   ...
   _can2_initTxBuffer(CAN_ANGLE_TX_BUF, 0);                                     // углы
   _can2_initTxBuffer(CAN_TEST1_TX_BUF, 0);                                      // тест для загрузки шины
   _can2_initTxBuffer(CAN_TEST2_TX_BUF, 0);                                      // тест для загрузки шины
   _can2_initTxBuffer(CAN_TEST3_TX_BUF, 0);                                      // тест для загрузки шины
...
//   MDR_CAN2->INT_RX |= 1<<CAN_TERM_RX_BUF;                                      // разрешение прерывания первого буфера
   ...
   // Передающий буфер - для углов
   MDR_CAN2->BUF_CON[CAN_ANGLE_TX_BUF] = 0<<1 |                                 // буфер работает на передачу
                                         1<<0;                                  // разрешение работы буфера

   // Передающий буфер - для теста
   MDR_CAN2->BUF_CON[CAN_TEST_TX_BUF] = 0<<1 |                                  // буфер работает на передачу
                                         1<<0;                                  // разрешение работы буфера
*/
