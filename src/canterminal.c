...
extern uint32_t operatingTimeAtBeginning_min;   // время наработки, мин. На момент включения питания
...
void process_rx_frame(uint8_t *buf, uint8_t len)
{
    ...
    else if(check_cmd("gtf", buf, len))                                         // время наработки formatted
    {
       ...
       timeM = operatingTimeAtBeginning_min + (timeS / 60);   // кол-во минут (>60)
       timeS = timeS % 60;                                    // кол-во секунд (0..59)
       
       timeH = timeM/60;                // кол-во часов
       timeM = timeM%60;                // кол-во минут (0..60)
              
       txtlen = sprintf((char*)txtbuf, "KT-109. Operating time %d:%d:%d.%d", timeH, timeM, timeS, timeMs);
       send_to_term(txtbuf, txtlen);   
    }
    else if(check_cmd("gt", buf, len))                                          // время наработки 
    {              
       extern uint32_t operatingTime_min;
       txtlen = sprintf((char*)txtbuf, "KT-109\n"
                                       "operatingTimeAtBeginning_min = %d\n"
                                       "operatingTime_min = %d\n"
                                       "system_time = %d\n",
                                        operatingTimeAtBeginning_min, operatingTime_min, system_time);
       send_to_term(txtbuf, txtlen);   
    }
}
